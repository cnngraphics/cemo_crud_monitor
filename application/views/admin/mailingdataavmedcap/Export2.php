<?php
	...
	<strong>'.LANG_REPORT_TABLE.'</strong> Mailingdataavmedcap</p>';
	$excel.='
    ...
    <tr>
	<td>MailingDataAvmedCAPRecord Id</td>
	<td>'.$rows->mailingDataAvmedCAPRecord_id.'</td>
  	</tr>
    <tr>
	<td>Generic1</td>
	<td>'.$rows->generic1.'</td>
   ...
   $excel.='</table>';
	...
	$filename1= 'mailingdataavmedcap_'.date('Y-m-d').'.doc';
	$filename2= 'mailingdataavmedcap_'.date('Y-m-d').'.xls';
	$pdf_output= 'mailingdataavmedcap_'.date('Y-m-d').'.pdf';
	if ($etype == 'word') 
    {
        MSWord($filename1);
        print $excel;
