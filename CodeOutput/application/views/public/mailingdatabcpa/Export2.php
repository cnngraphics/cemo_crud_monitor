<?php
	...
	<strong>'.LANG_REPORT_TABLE.'</strong> Mailingdatabcpa</p>';
	$excel.='
    ...
    <tr>
	<td>MailingDataBCPARecord Id</td>
	<td>'.$rows->mailingDataBCPARecord_id.'</td>
  	</tr>
    <tr>
	<td>Generic1</td>
	<td>'.$rows->generic1.'</td>
   ...
   $excel.='</table>';
	...
	$filename1= 'mailingdatabcpa_'.date('Y-m-d').'.doc';
	$filename2= 'mailingdatabcpa_'.date('Y-m-d').'.xls';
	$pdf_output= 'mailingdatabcpa_'.date('Y-m-d').'.pdf';
	if ($etype == 'word') 
    {
        MSWord($filename1);
        print $excel;
