<?php
	...
	*/
	...
	if($_POST){	
	if(!isset($_FILES[$dfile])){die("File is empty!");}
	if($_FILES[$dfile]['error']){die($this->upload_errors($_FILES[$dfile]['error']));}
   ...
   if(move_uploaded_file($_FILES[$dfile]["tmp_name"], $path_main . $Hfilename ))
   {
	...
	function randNum(){
		  $code = '';
	  for($x = 0; $x<5; $x++) {
		  $code .= ''.substr(rand(0,999999999),2,3);
	  }
	  $code = substr($code,1);
	  return $code;
  	}
