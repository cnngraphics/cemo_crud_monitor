	...
	*/
	...
	include(APP_FOLDER.'/views/public/mailingprintreadyfile/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->mailingprintreadyfile_model->SelectAll();
	include(APP_FOLDER.'/views/public/mailingprintreadyfile/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->mailingprintreadyfile_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingprintreadyfile/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/public/mailingprintreadyfile/Add.php');
	}
	...
	json_send(''.H_CLIENT.'&view=mailingprintreadyfile&do=viewall&msg=add');
	...
	include(APP_FOLDER.'/views/public/mailingprintreadyfile/Update.php');
	}
	...
	json_send(''.H_CLIENT.'&view=mailingprintreadyfile&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->mailingprintreadyfile_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingprintreadyfile/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->mailingprintreadyfile_model->TruncateTable(''.H_CLIENT.'&view=mailingprintreadyfile&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/public/mailingprintreadyfile/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->mailingprintreadyfile_model->Delete(get('id'),''.H_CLIENT.'&view=mailingprintreadyfile&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_CLIENT.'&view=mailingprintreadyfile&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
