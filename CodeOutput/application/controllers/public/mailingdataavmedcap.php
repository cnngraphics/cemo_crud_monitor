	...
	/*
	* =======================================================================
	* FILE NAME:        mailingdataavmedcap.php
	* DATE CREATED:  	28-02-2020
	* FOR TABLE:  		mailingdataavmedcap
	...
	*/
	...
	include(APP_FOLDER.'/models/objects/mailingdataavmedcap.php');
	
	class mailingdataavmedcap_controller {
	public $mailingdataavmedcap_model;
	
	public function __construct()  
    {  
        $this->mailingdataavmedcap_model = new mailingdataavmedcap_model();
    } 
	
	public function invoke_mailingdataavmedcap()
	{
	
	//SELECT ALL //////////////////////////////////	
	if(get('do')=='viewall'){
	if(PAGINATION_TYPE=='Normal'){
	$result = $this->mailingdataavmedcap_model->SelectAll(RECORD_PER_PAGE);
	//Accept get url  e.g (index.php?id=1&cat=2...)
	$paging = pagination($this->mailingdataavmedcap_model->CountRow(),RECORD_PER_PAGE,''.H_ADMIN.'&view=mailingdataavmedcap&do=viewall');
	}else{
	$result = $this->mailingdataavmedcap_model->SelectAll();	
	}
	include(APP_FOLDER.'/views/public/mailingdataavmedcap/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->mailingdataavmedcap_model->SelectAll();
	include(APP_FOLDER.'/views/public/mailingdataavmedcap/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->mailingdataavmedcap_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedcap/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	elseif(get('do')=='autosearch'){
	$qstring = post('qstring');
	if(strlen($qstring) >0) {
	$autosearch = $this->mailingdataavmedcap_model->AutoSearch(trim($qstring),10,'firstName');
	echo' <div class=widget><ul class="list-group">';
	foreach ($autosearch as $srow) {
	echo '<span class="searchheading"><a href="'.H_ADMIN.'&view=mailingdataavmedcap&id='.$srow->id.'&do=details"><li class="list-group-item">'. $srow->firstName.'</li></a>
	</span>';
	}
	echo '</ul></div>';
	}
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/public/mailingdataavmedcap/Add.php');
	}
	...
	elseif (post('mailingDataAvmedCAPRecord_id')==''){
	json_error('The field mailingDataAvmedCAPRecord id cannot be empty!');
	}
	...
	$this->mailingdataavmedcap_model->Insert(post('firstName'),post('lastName'),post('middleName'),post('fullName'),post('address1'),post('mmAddress1'),post('address2'),post('mmAddress2'),post('city'),post('mmCity'),post('state'),post('mmState'),post('zipCode'),post('mmZipCode'),post('country'),post('mmCountry'),post('mmImb'),post('mmOpt'),post('mmReturnCode'),post('mmDpv'),post('mmOrder'),post('mmCOAMoveToDate'),post('mmCOAMoveType'),post('pdfLocation'),post('pdfTotPages'),post('documentDate'),post('seqNumber'),post('lastPage'),post('reprint'),post('checkNumber'),post('checkDate'),post('checkAmount'),post('mailingDataFile_id'),post('mailingPrintReadyFile_id'),post('mailingDataAvmedCAPRecord_id'),post('generic1'),post('generic2'),post('mmImbDigi'));
	json_send(''.H_CLIENT.'&view=mailingdataavmedcap&do=viewall&msg=add');
	...
	//UPDATE //////////////////////////////////////////////////
	elseif(get('do')=='update'){$rows = $this->mailingdataavmedcap_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedcap/Update.php');
	}
	...
	$this->mailingdataavmedcap_model->Update(post('firstName'),post('lastName'),post('middleName'),post('fullName'),post('address1'),post('mmAddress1'),post('address2'),post('mmAddress2'),post('city'),post('mmCity'),post('state'),post('mmState'),post('zipCode'),post('mmZipCode'),post('country'),post('mmCountry'),post('mmImb'),post('mmOpt'),post('mmReturnCode'),post('mmDpv'),post('mmOrder'),post('mmCOAMoveToDate'),post('mmCOAMoveType'),post('pdfLocation'),post('pdfTotPages'),post('documentDate'),post('seqNumber'),post('lastPage'),post('reprint'),post('checkNumber'),post('checkDate'),post('checkAmount'),post('mailingDataFile_id'),post('mailingPrintReadyFile_id'),post('mailingDataAvmedCAPRecord_id'),post('generic1'),post('generic2'),post('mmImbDigi'),post('id'));
	json_send(''.H_CLIENT.'&view=mailingdataavmedcap&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->mailingdataavmedcap_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedcap/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->mailingdataavmedcap_model->TruncateTable(''.H_CLIENT.'&view=mailingdataavmedcap&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/public/mailingdataavmedcap/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->mailingdataavmedcap_model->Delete(get('id'),''.H_CLIENT.'&view=mailingdataavmedcap&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_CLIENT.'&view=mailingdataavmedcap&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
