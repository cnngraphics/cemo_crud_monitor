	...
	/*
	* =======================================================================
	* FILE NAME:        mailingdatamyron.php
	...
	*/
	...
	include(APP_FOLDER.'/models/objects/mailingdatamyron.php');
	
	class mailingdatamyron_controller {
	public $mailingdatamyron_model;
	
	public function __construct()  
    {  
        $this->mailingdatamyron_model = new mailingdatamyron_model();
    } 
	
	public function invoke_mailingdatamyron()
	{
	
	//SELECT ALL //////////////////////////////////	
	if(get('do')=='viewall'){
	if(PAGINATION_TYPE=='Normal'){
	$result = $this->mailingdatamyron_model->SelectAll(RECORD_PER_PAGE);
	//Accept get url  e.g (index.php?id=1&cat=2...)
	$paging = pagination($this->mailingdatamyron_model->CountRow(),RECORD_PER_PAGE,''.H_ADMIN.'&view=mailingdatamyron&do=viewall');
	}else{
	$result = $this->mailingdatamyron_model->SelectAll();	
	}
	include(APP_FOLDER.'/views/public/mailingdatamyron/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->mailingdatamyron_model->SelectAll();
	include(APP_FOLDER.'/views/public/mailingdatamyron/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->mailingdatamyron_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdatamyron/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	elseif(get('do')=='autosearch'){
	$qstring = post('qstring');
	if(strlen($qstring) >0) {
	$autosearch = $this->mailingdatamyron_model->AutoSearch(trim($qstring),10,'firstName');
	echo' <div class=widget><ul class="list-group">';
	foreach ($autosearch as $srow) {
	echo '<span class="searchheading"><a href="'.H_ADMIN.'&view=mailingdatamyron&id='.$srow->id.'&do=details"><li class="list-group-item">'. $srow->firstName.'</li></a>
	</span>';
	}
	echo '</ul></div>';
	}
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/public/mailingdatamyron/Add.php');
	}
	...
	//UPDATE //////////////////////////////////////////////////
	elseif(get('do')=='update'){$rows = $this->mailingdatamyron_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdatamyron/Update.php');
	}
	...
	$rows = $this->mailingdatamyron_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdatamyron/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->mailingdatamyron_model->TruncateTable(''.H_CLIENT.'&view=mailingdatamyron&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/public/mailingdatamyron/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->mailingdatamyron_model->Delete(get('id'),''.H_CLIENT.'&view=mailingdatamyron&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_CLIENT.'&view=mailingdatamyron&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
