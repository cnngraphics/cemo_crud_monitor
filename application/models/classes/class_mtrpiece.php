
	<?php
	/*
	* =======================================================================
	* CLASSNAME:        mtrpiece
	* DATE CREATED:  	28-02-2020
	* FOR TABLE:  		mtrpiece
	* PRODUCED BY:		HEZECOM UltimateSpeed PHP CODE GENERATOR
	* AUTHOR:			Hezecom (http://hezecom.com) info@hezecom.net
	* IMPORTANT:		
	* 'post()' is a defined function located @ libries/funtions.php
	* =======================================================================
	*/
	if(!defined('VALID_DIR')) die('You are not allowed to execute this file directly');
	//Begin class
	
	class mtrpiece
	{
	public $id;
	public $sourceFile; 
	public $edocJobId; 
	public $edocMailingGroupId; 
	public $edocSubmitterCrid; 
	public $handlingEventType; 
	public $handlingEventTypeDescription; 
	public $idTag; 
	public $imb; 
	public $imbMid; 
	public $imbRoutingCode; 
	public $imbSerialNumber; 
	public $imbStid; 
	public $imbTrackingCode; 
	public $ldeDeliveryMode; 
	public $ldeInventoryMethod; 
	public $ldeTriggerMethod; 
	public $machineId; 
	public $machineName; 
	public $mailClassDescription; 
	public $mailShapeDescription; 
	public $parentContainerEdocContainerId; 
	public $parentTrayEdocContainerId; 
	public $predictedDeliveryDate; 
	public $routingCodeImbMatchingPortion; 
	public $scanDatetime; 
	public $scanEventCode; 
	public $scanFacilityCity; 
	public $scanFacilityName; 
	public $scanFacilityState; 
	public $scanFacilityZip; 
	public $scanLocaleKey; 
	public $scannerType; 
	public $startTheClockDate; 
	
	//Get Methods
	
	public function getid()
	{
	return $this->id;
	}
	
	public function getsourceFile()
	{
	return $this->sourceFile;
	}
	
	public function getedocJobId()
	{
	return $this->edocJobId;
	}
	
	public function getedocMailingGroupId()
	{
	return $this->edocMailingGroupId;
	}
	
	public function getedocSubmitterCrid()
	{
	return $this->edocSubmitterCrid;
	}
	
	public function gethandlingEventType()
	{
	return $this->handlingEventType;
	}
	
	public function gethandlingEventTypeDescription()
	{
	return $this->handlingEventTypeDescription;
	}
	
	public function getidTag()
	{
	return $this->idTag;
	}
	
	public function getimb()
	{
	return $this->imb;
	}
	
	public function getimbMid()
	{
	return $this->imbMid;
	}
	
	public function getimbRoutingCode()
	{
	return $this->imbRoutingCode;
	}
	
	public function getimbSerialNumber()
	{
	return $this->imbSerialNumber;
	}
	
	public function getimbStid()
	{
	return $this->imbStid;
	}
	
	public function getimbTrackingCode()
	{
	return $this->imbTrackingCode;
	}
	
	public function getldeDeliveryMode()
	{
	return $this->ldeDeliveryMode;
	}
	
	public function getldeInventoryMethod()
	{
	return $this->ldeInventoryMethod;
	}
	
	public function getldeTriggerMethod()
	{
	return $this->ldeTriggerMethod;
	}
	
	public function getmachineId()
	{
	return $this->machineId;
	}
	
	public function getmachineName()
	{
	return $this->machineName;
	}
	
	public function getmailClassDescription()
	{
	return $this->mailClassDescription;
	}
	
	public function getmailShapeDescription()
	{
	return $this->mailShapeDescription;
	}
	
	public function getparentContainerEdocContainerId()
	{
	return $this->parentContainerEdocContainerId;
	}
	
	public function getparentTrayEdocContainerId()
	{
	return $this->parentTrayEdocContainerId;
	}
	
	public function getpredictedDeliveryDate()
	{
	return $this->predictedDeliveryDate;
	}
	
	public function getroutingCodeImbMatchingPortion()
	{
	return $this->routingCodeImbMatchingPortion;
	}
	
	public function getscanDatetime()
	{
	return $this->scanDatetime;
	}
	
	public function getscanEventCode()
	{
	return $this->scanEventCode;
	}
	
	public function getscanFacilityCity()
	{
	return $this->scanFacilityCity;
	}
	
	public function getscanFacilityName()
	{
	return $this->scanFacilityName;
	}
	
	public function getscanFacilityState()
	{
	return $this->scanFacilityState;
	}
	
	public function getscanFacilityZip()
	{
	return $this->scanFacilityZip;
	}
	
	public function getscanLocaleKey()
	{
	return $this->scanLocaleKey;
	}
	
	public function getscannerType()
	{
	return $this->scannerType;
	}
	
	public function getstartTheClockDate()
	{
	return $this->startTheClockDate;
	}
	
	// Set Methods
	
	public function setid($val)
	{
	$this->id =  $val;
	}
	
	public function setsourceFile($val)
	{
	$this->sourceFile =  $val;
	}
	
	public function setedocJobId($val)
	{
	$this->edocJobId =  $val;
	}
	
	public function setedocMailingGroupId($val)
	{
	$this->edocMailingGroupId =  $val;
	}
	
	public function setedocSubmitterCrid($val)
	{
	$this->edocSubmitterCrid =  $val;
	}
	
	public function sethandlingEventType($val)
	{
	$this->handlingEventType =  $val;
	}
	
	public function sethandlingEventTypeDescription($val)
	{
	$this->handlingEventTypeDescription =  $val;
	}
	
	public function setidTag($val)
	{
	$this->idTag =  $val;
	}
	
	public function setimb($val)
	{
	$this->imb =  $val;
	}
	
	public function setimbMid($val)
	{
	$this->imbMid =  $val;
	}
	
	public function setimbRoutingCode($val)
	{
	$this->imbRoutingCode =  $val;
	}
	
	public function setimbSerialNumber($val)
	{
	$this->imbSerialNumber =  $val;
	}
	
	public function setimbStid($val)
	{
	$this->imbStid =  $val;
	}
	
	public function setimbTrackingCode($val)
	{
	$this->imbTrackingCode =  $val;
	}
	
	public function setldeDeliveryMode($val)
	{
	$this->ldeDeliveryMode =  $val;
	}
	
	public function setldeInventoryMethod($val)
	{
	$this->ldeInventoryMethod =  $val;
	}
	
	public function setldeTriggerMethod($val)
	{
	$this->ldeTriggerMethod =  $val;
	}
	
	public function setmachineId($val)
	{
	$this->machineId =  $val;
	}
	
	public function setmachineName($val)
	{
	$this->machineName =  $val;
	}
	
	public function setmailClassDescription($val)
	{
	$this->mailClassDescription =  $val;
	}
	
	public function setmailShapeDescription($val)
	{
	$this->mailShapeDescription =  $val;
	}
	
	public function setparentContainerEdocContainerId($val)
	{
	$this->parentContainerEdocContainerId =  $val;
	}
	
	public function setparentTrayEdocContainerId($val)
	{
	$this->parentTrayEdocContainerId =  $val;
	}
	
	public function setpredictedDeliveryDate($val)
	{
	$this->predictedDeliveryDate =  $val;
	}
	
	public function setroutingCodeImbMatchingPortion($val)
	{
	$this->routingCodeImbMatchingPortion =  $val;
	}
	
	public function setscanDatetime($val)
	{
	$this->scanDatetime =  $val;
	}
	
	public function setscanEventCode($val)
	{
	$this->scanEventCode =  $val;
	}
	
	public function setscanFacilityCity($val)
	{
	$this->scanFacilityCity =  $val;
	}
	
	public function setscanFacilityName($val)
	{
	$this->scanFacilityName =  $val;
	}
	
	public function setscanFacilityState($val)
	{
	$this->scanFacilityState =  $val;
	}
	
	public function setscanFacilityZip($val)
	{
	$this->scanFacilityZip =  $val;
	}
	
	public function setscanLocaleKey($val)
	{
	$this->scanLocaleKey =  $val;
	}
	
	public function setscannerType($val)
	{
	$this->scannerType =  $val;
	}
	
	public function setstartTheClockDate($val)
	{
	$this->startTheClockDate =  $val;
	}
	}