	...
	/*
	* =======================================================================
	* FILE NAME:        mailingdataavmed.php
	...
	*/
	...
	include(APP_FOLDER.'/models/objects/mailingdataavmed.php');
	
	class mailingdataavmed_controller {
	public $mailingdataavmed_model;
	
	public function __construct()  
    {  
        $this->mailingdataavmed_model = new mailingdataavmed_model();
    } 
	
	public function invoke_mailingdataavmed()
	{
	
	//SELECT ALL //////////////////////////////////	
	if(get('do')=='viewall'){
	if(PAGINATION_TYPE=='Normal'){
	$result = $this->mailingdataavmed_model->SelectAll(RECORD_PER_PAGE);
	//Accept get url  e.g (index.php?id=1&cat=2...)
	$paging = pagination($this->mailingdataavmed_model->CountRow(),RECORD_PER_PAGE,''.H_ADMIN.'&view=mailingdataavmed&do=viewall');
	}else{
	$result = $this->mailingdataavmed_model->SelectAll();	
	}
	include(APP_FOLDER.'/views/public/mailingdataavmed/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->mailingdataavmed_model->SelectAll();
	include(APP_FOLDER.'/views/public/mailingdataavmed/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->mailingdataavmed_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmed/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	elseif(get('do')=='autosearch'){
	$qstring = post('qstring');
	if(strlen($qstring) >0) {
	$autosearch = $this->mailingdataavmed_model->AutoSearch(trim($qstring),10,'firstName');
	echo' <div class=widget><ul class="list-group">';
	foreach ($autosearch as $srow) {
	echo '<span class="searchheading"><a href="'.H_ADMIN.'&view=mailingdataavmed&id='.$srow->id.'&do=details"><li class="list-group-item">'. $srow->firstName.'</li></a>
	</span>';
	}
	echo '</ul></div>';
	}
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/public/mailingdataavmed/Add.php');
	}
	...
	elseif (post('mailingDataAvmedRecord_id')==''){
	json_error('The field mailingDataAvmedRecord id cannot be empty!');
	}
	...
	$this->mailingdataavmed_model->Insert(post('firstName'),post('lastName'),post('middleName'),post('fullName'),post('address1'),post('mmAddress1'),post('address2'),post('mmAddress2'),post('city'),post('mmCity'),post('state'),post('mmState'),post('zipCode'),post('mmZipCode'),post('country'),post('mmCountry'),post('mmImb'),post('mmOpt'),post('mmReturnCode'),post('mmDpv'),post('mmOrder'),post('pdfLocation'),post('documentDate'),post('mailingDataFile_id'),post('mailingPrintReadyFile_id'),post('mailingDataAvmedRecord_id'),post('pdfTotPages'),post('mmCOAMoveToDate'),post('mmCOAMoveType'),post('seqNumber'),post('reprint'),post('generic1'),post('generic2'),post('mmImbDigi'));
	json_send(''.H_CLIENT.'&view=mailingdataavmed&do=viewall&msg=add');
	...
	//UPDATE //////////////////////////////////////////////////
	elseif(get('do')=='update'){$rows = $this->mailingdataavmed_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmed/Update.php');
	}
	...
	$this->mailingdataavmed_model->Update(post('firstName'),post('lastName'),post('middleName'),post('fullName'),post('address1'),post('mmAddress1'),post('address2'),post('mmAddress2'),post('city'),post('mmCity'),post('state'),post('mmState'),post('zipCode'),post('mmZipCode'),post('country'),post('mmCountry'),post('mmImb'),post('mmOpt'),post('mmReturnCode'),post('mmDpv'),post('mmOrder'),post('pdfLocation'),post('documentDate'),post('mailingDataFile_id'),post('mailingPrintReadyFile_id'),post('mailingDataAvmedRecord_id'),post('pdfTotPages'),post('mmCOAMoveToDate'),post('mmCOAMoveType'),post('seqNumber'),post('reprint'),post('generic1'),post('generic2'),post('mmImbDigi'),post('id'));
	json_send(''.H_CLIENT.'&view=mailingdataavmed&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->mailingdataavmed_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmed/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->mailingdataavmed_model->TruncateTable(''.H_CLIENT.'&view=mailingdataavmed&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/public/mailingdataavmed/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->mailingdataavmed_model->Delete(get('id'),''.H_CLIENT.'&view=mailingdataavmed&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_CLIENT.'&view=mailingdataavmed&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
