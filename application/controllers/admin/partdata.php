	...
	*/
	...
	include(APP_FOLDER.'/views/admin/partdata/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->partdata_model->SelectAll();
	include(APP_FOLDER.'/views/admin/partdata/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->partdata_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/partdata/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/admin/partdata/Add.php');
	}
	...
	json_send(''.H_ADMIN.'&view=partdata&do=viewall&msg=add');
	...
	include(APP_FOLDER.'/views/admin/partdata/Update.php');
	}
	...
	json_send(''.H_ADMIN.'&view=partdata&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->partdata_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/partdata/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->partdata_model->TruncateTable(''.H_ADMIN.'&view=partdata&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/admin/partdata/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->partdata_model->Delete(get('id'),''.H_ADMIN.'&view=partdata&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_ADMIN.'&view=partdata&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
