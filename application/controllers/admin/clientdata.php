	...
	*/
	...
	include(APP_FOLDER.'/views/admin/clientdata/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->clientdata_model->SelectAll();
	include(APP_FOLDER.'/views/admin/clientdata/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->clientdata_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/clientdata/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/admin/clientdata/Add.php');
	}
	...
	json_send(''.H_ADMIN.'&view=clientdata&do=viewall&msg=add');
	...
	include(APP_FOLDER.'/views/admin/clientdata/Update.php');
	}
	...
	json_send(''.H_ADMIN.'&view=clientdata&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->clientdata_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/clientdata/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->clientdata_model->TruncateTable(''.H_ADMIN.'&view=clientdata&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/admin/clientdata/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->clientdata_model->Delete(get('id'),''.H_ADMIN.'&view=clientdata&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_ADMIN.'&view=clientdata&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
