	...
	*/
	...
	include(APP_FOLDER.'/views/public/jobdata/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->jobdata_model->SelectAll();
	include(APP_FOLDER.'/views/public/jobdata/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->jobdata_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/jobdata/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/public/jobdata/Add.php');
	}
	...
	json_send(''.H_CLIENT.'&view=jobdata&do=viewall&msg=add');
	...
	include(APP_FOLDER.'/views/public/jobdata/Update.php');
	}
	...
	json_send(''.H_CLIENT.'&view=jobdata&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->jobdata_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/jobdata/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->jobdata_model->TruncateTable(''.H_CLIENT.'&view=jobdata&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/public/jobdata/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->jobdata_model->Delete(get('id'),''.H_CLIENT.'&view=jobdata&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_CLIENT.'&view=jobdata&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
