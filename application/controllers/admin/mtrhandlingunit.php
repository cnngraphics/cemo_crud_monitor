	...
	*/
	...
	include(APP_FOLDER.'/views/admin/mtrhandlingunit/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->mtrhandlingunit_model->SelectAll();
	include(APP_FOLDER.'/views/admin/mtrhandlingunit/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->mtrhandlingunit_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/mtrhandlingunit/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/admin/mtrhandlingunit/Add.php');
	}
	...
	json_send(''.H_ADMIN.'&view=mtrhandlingunit&do=viewall&msg=add');
	...
	include(APP_FOLDER.'/views/admin/mtrhandlingunit/Update.php');
	}
	...
	json_send(''.H_ADMIN.'&view=mtrhandlingunit&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->mtrhandlingunit_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/mtrhandlingunit/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->mtrhandlingunit_model->TruncateTable(''.H_ADMIN.'&view=mtrhandlingunit&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/admin/mtrhandlingunit/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->mtrhandlingunit_model->Delete(get('id'),''.H_ADMIN.'&view=mtrhandlingunit&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_ADMIN.'&view=mtrhandlingunit&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
