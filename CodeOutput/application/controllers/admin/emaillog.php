	...
	*/
	...
	include(APP_FOLDER.'/views/admin/emaillog/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->emaillog_model->SelectAll();
	include(APP_FOLDER.'/views/admin/emaillog/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->emaillog_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/emaillog/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/admin/emaillog/Add.php');
	}
	...
	json_send(''.H_ADMIN.'&view=emaillog&do=viewall&msg=add');
	...
	include(APP_FOLDER.'/views/admin/emaillog/Update.php');
	}
	...
	json_send(''.H_ADMIN.'&view=emaillog&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->emaillog_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/emaillog/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->emaillog_model->TruncateTable(''.H_ADMIN.'&view=emaillog&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/admin/emaillog/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->emaillog_model->Delete(get('id'),''.H_ADMIN.'&view=emaillog&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_ADMIN.'&view=emaillog&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
