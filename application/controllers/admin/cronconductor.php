	...
	*/
	...
	include(APP_FOLDER.'/views/admin/cronconductor/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->cronconductor_model->SelectAll();
	include(APP_FOLDER.'/views/admin/cronconductor/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->cronconductor_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/cronconductor/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/admin/cronconductor/Add.php');
	}
	...
	json_send(''.H_ADMIN.'&view=cronconductor&do=viewall&msg=add');
	...
	include(APP_FOLDER.'/views/admin/cronconductor/Update.php');
	}
	...
	json_send(''.H_ADMIN.'&view=cronconductor&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->cronconductor_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/admin/cronconductor/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->cronconductor_model->TruncateTable(''.H_ADMIN.'&view=cronconductor&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/admin/cronconductor/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->cronconductor_model->Delete(get('id'),''.H_ADMIN.'&view=cronconductor&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_ADMIN.'&view=cronconductor&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
