<?php
	...
	<strong>'.LANG_REPORT_TABLE.'</strong> Mailingdataavmedeob</p>';
	$excel.='
    ...
    <tr>
	<td>MailingDataAvmedEOBRecord Id</td>
	<td>'.$rows->mailingDataAvmedEOBRecord_id.'</td>
  	</tr>
    <tr>
	<td>Generic1</td>
	<td>'.$rows->generic1.'</td>
   ...
   $excel.='</table>';
	...
	$filename1= 'mailingdataavmedeob_'.date('Y-m-d').'.doc';
	$filename2= 'mailingdataavmedeob_'.date('Y-m-d').'.xls';
	$pdf_output= 'mailingdataavmedeob_'.date('Y-m-d').'.pdf';
	if ($etype == 'word') 
    {
        MSWord($filename1);
        print $excel;
