	...
	/*
	* =======================================================================
	* FILE NAME:        mailingdataavmedeob.php
	...
	*/
	...
	include(APP_FOLDER.'/models/objects/mailingdataavmedeob.php');
	
	class mailingdataavmedeob_controller {
	public $mailingdataavmedeob_model;
	
	public function __construct()  
    {  
        $this->mailingdataavmedeob_model = new mailingdataavmedeob_model();
    } 
	
	public function invoke_mailingdataavmedeob()
	{
	
	//SELECT ALL //////////////////////////////////	
	if(get('do')=='viewall'){
	if(PAGINATION_TYPE=='Normal'){
	$result = $this->mailingdataavmedeob_model->SelectAll(RECORD_PER_PAGE);
	//Accept get url  e.g (index.php?id=1&cat=2...)
	$paging = pagination($this->mailingdataavmedeob_model->CountRow(),RECORD_PER_PAGE,''.H_ADMIN.'&view=mailingdataavmedeob&do=viewall');
	}else{
	$result = $this->mailingdataavmedeob_model->SelectAll();	
	}
	include(APP_FOLDER.'/views/public/mailingdataavmedeob/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->mailingdataavmedeob_model->SelectAll();
	include(APP_FOLDER.'/views/public/mailingdataavmedeob/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->mailingdataavmedeob_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedeob/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	elseif(get('do')=='autosearch'){
	$qstring = post('qstring');
	if(strlen($qstring) >0) {
	$autosearch = $this->mailingdataavmedeob_model->AutoSearch(trim($qstring),10,'firstName');
	echo' <div class=widget><ul class="list-group">';
	foreach ($autosearch as $srow) {
	echo '<span class="searchheading"><a href="'.H_ADMIN.'&view=mailingdataavmedeob&id='.$srow->id.'&do=details"><li class="list-group-item">'. $srow->firstName.'</li></a>
	</span>';
	}
	echo '</ul></div>';
	}
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/public/mailingdataavmedeob/Add.php');
	}
	...
	elseif (post('mailingDataAvmedEOBRecord_id')==''){
	json_error('The field mailingDataAvmedEOBRecord id cannot be empty!');
	}
	...
	//UPDATE //////////////////////////////////////////////////
	elseif(get('do')=='update'){$rows = $this->mailingdataavmedeob_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedeob/Update.php');
	}
	...
	$this->mailingdataavmedeob_model->Update(post('firstName'),post('lastName'),post('middleName'),post('fullName'),post('address1'),post('mmAddress1'),post('address2'),post('mmAddress2'),post('city'),post('mmCity'),post('state'),post('mmState'),post('zipCode'),post('mmZipCode'),post('country'),post('mmCountry'),post('mmImb'),post('mmOpt'),post('mmReturnCode'),post('mmDpv'),post('mmOrder'),post('mmCOAMoveToDate'),post('mmCOAMoveType'),post('pdfLocation'),post('pdfTotPages'),post('documentDate'),post('seqNumber'),post('lastPage'),post('reprint'),post('avmedID'),post('mailingDataFile_id'),post('mailingPrintReadyFile_id'),post('mailingDataAvmedEOBRecord_id'),post('generic1'),post('generic2'),post('mmImbDigi'),post('id'));
	json_send(''.H_CLIENT.'&view=mailingdataavmedeob&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->mailingdataavmedeob_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedeob/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->mailingdataavmedeob_model->TruncateTable(''.H_CLIENT.'&view=mailingdataavmedeob&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/public/mailingdataavmedeob/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->mailingdataavmedeob_model->Delete(get('id'),''.H_CLIENT.'&view=mailingdataavmedeob&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_CLIENT.'&view=mailingdataavmedeob&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
