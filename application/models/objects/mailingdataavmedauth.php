	...
	/*
	* =======================================================================
	* CLASSNAME:        mailingdataavmedauth_model
	...
	*/
	...
	include_once(APP_FOLDER.'/models/classes/class_mailingdataavmedauth.php');
	
	class mailingdataavmedauth_model{
	...
	return HDB::hus()->Hselect("mailingdataavmedauth LIMIT {$startpg} , {$limit}");
	}else{
	return HDB::hus()->Hselect("mailingdataavmedauth");	
	}
	}
	...
	{
	return HDB::hus()->Hcount("mailingdataavmedauth");
	}
	...
	{
	$bind = array(":id" =>$id);
	return HDB::hus()->Hone("mailingdataavmedauth","id=:id",$bind);
	}
	...
	return HDB::hus()->Hselect("mailingdataavmedauth","$where LIKE :svalue LIMIT $limit",$bind);		
	}
	
	// TRUNCATE TABLE
	public function TruncateTable($redirect_to)
	{
   	$sql=HDB::hus()->prepare("TRUNCATE mailingdataavmedauth");
	$sql->execute();
	send_to($redirect_to);
	}
	...
	{
	$bind = array(":id" =>$id);
	HDB::hus()->Hdelete("mailingdataavmedauth","id=:id",$bind);
	send_to($redirect_to);
	}
	...
	HDB::hus()->Hinsert('mailingdataavmedauth', $values);
	}
	...
	HDB::hus()->Hupdate('mailingdataavmedauth',$sql,$data);
