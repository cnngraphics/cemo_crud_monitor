	...
	/*
	* =======================================================================
	* FILE NAME:        mailingdataavmedmedeobquarterly.php
	...
	*/
	...
	include(APP_FOLDER.'/models/objects/mailingdataavmedmedeobquarterly.php');
	
	class mailingdataavmedmedeobquarterly_controller {
	public $mailingdataavmedmedeobquarterly_model;
	
	public function __construct()  
    {  
        $this->mailingdataavmedmedeobquarterly_model = new mailingdataavmedmedeobquarterly_model();
    } 
	
	public function invoke_mailingdataavmedmedeobquarterly()
	{
	
	//SELECT ALL //////////////////////////////////	
	if(get('do')=='viewall'){
	if(PAGINATION_TYPE=='Normal'){
	$result = $this->mailingdataavmedmedeobquarterly_model->SelectAll(RECORD_PER_PAGE);
	//Accept get url  e.g (index.php?id=1&cat=2...)
	$paging = pagination($this->mailingdataavmedmedeobquarterly_model->CountRow(),RECORD_PER_PAGE,''.H_ADMIN.'&view=mailingdataavmedmedeobquarterly&do=viewall');
	}else{
	$result = $this->mailingdataavmedmedeobquarterly_model->SelectAll();	
	}
	include(APP_FOLDER.'/views/public/mailingdataavmedmedeobquarterly/View.php');
	}
	...
	if(get('do')=='export'){
	$result = $this->mailingdataavmedmedeobquarterly_model->SelectAll();
	include(APP_FOLDER.'/views/public/mailingdataavmedmedeobquarterly/Export.php');
	}
	
	//Expeort2
	elseif(get('do')=='export2'){
	$rows = $this->mailingdataavmedmedeobquarterly_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedmedeobquarterly/Export2.php');
	}
	//SEARCH SUGGEST ////////////////////////////////////////////////////	
	elseif(get('do')=='autosearch'){
	$qstring = post('qstring');
	if(strlen($qstring) >0) {
	$autosearch = $this->mailingdataavmedmedeobquarterly_model->AutoSearch(trim($qstring),10,'firstName');
	echo' <div class=widget><ul class="list-group">';
	foreach ($autosearch as $srow) {
	echo '<span class="searchheading"><a href="'.H_ADMIN.'&view=mailingdataavmedmedeobquarterly&id='.$srow->id.'&do=details"><li class="list-group-item">'. $srow->firstName.'</li></a>
	</span>';
	}
	echo '</ul></div>';
	}
	...
	//ADD //////////////////////////////////////////////////
	elseif(get('do')=='add'){
	include(APP_FOLDER.'/views/public/mailingdataavmedmedeobquarterly/Add.php');
	}
	...
	//UPDATE //////////////////////////////////////////////////
	elseif(get('do')=='update'){$rows = $this->mailingdataavmedmedeobquarterly_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedmedeobquarterly/Update.php');
	}
	...
	$this->mailingdataavmedmedeobquarterly_model->Update(post('firstName'),post('lastName'),post('middleName'),post('fullName'),post('address1'),post('mmAddress1'),post('address2'),post('mmAddress2'),post('city'),post('mmCity'),post('state'),post('mmState'),post('zipCode'),post('mmZipCode'),post('country'),post('mmCountry'),post('mmImb'),post('mmOpt'),post('mmReturnCode'),post('mmDpv'),post('mmOrder'),post('mmCOAMoveToDate'),post('mmCOAMoveType'),post('pdfLocation'),post('pdfTotPages'),post('documentDate'),post('seqNumber'),post('lastPage'),post('reprint'),post('avmedID'),post('mailingDataFile_id'),post('mailingPrintReadyFile_id'),post('mailingDataRecord_id'),post('generic1'),post('generic2'),post('mmImbDigi'),post('id'));
	json_send(''.H_CLIENT.'&view=mailingdataavmedmedeobquarterly&id='.post('id').'&do=details&msg=update');
	json_success('Process Completed');
	}
	}
	}
	...
	$rows = $this->mailingdataavmedmedeobquarterly_model->SelectOne(get('id'));
	include(APP_FOLDER.'/views/public/mailingdataavmedmedeobquarterly/Details.php');
	}
	
	//TRUNCATE ///////////////////////////////////////////////
	elseif(get('do')=='truncate'){
	$this->mailingdataavmedmedeobquarterly_model->TruncateTable(''.H_CLIENT.'&view=mailingdataavmedmedeobquarterly&do=viewall&msg=truncate');
	include(APP_FOLDER.'/views/public/mailingdataavmedmedeobquarterly/View.php');
	}
	...
	$dfile=get('dfile');
		if(get('id') and $dfile==''){
	$del = $this->mailingdataavmedmedeobquarterly_model->Delete(get('id'),''.H_CLIENT.'&view=mailingdataavmedmedeobquarterly&do=viewall&msg=delete');
	}
	...
	delete_files(UPLOAD_PATH.get('dfile'));
	delete_files(THUMB_PATH.get('dfile'));
	send_to(''.H_CLIENT.'&view=mailingdataavmedmedeobquarterly&id='.get('id').'&do=update&msg=delete');
	}
	}
	}//end invoke
	}//end class
