	...
	/*
	* =======================================================================
	* CLASSNAME:        mailingdataavmedrenewal_model
	...
	*/
	...
	include_once(APP_FOLDER.'/models/classes/class_mailingdataavmedrenewal.php');
	
	class mailingdataavmedrenewal_model{
	...
	return HDB::hus()->Hselect("mailingdataavmedrenewal LIMIT {$startpg} , {$limit}");
	}else{
	return HDB::hus()->Hselect("mailingdataavmedrenewal");	
	}
	}
	...
	{
	return HDB::hus()->Hcount("mailingdataavmedrenewal");
	}
	...
	{
	$bind = array(":id" =>$id);
	return HDB::hus()->Hone("mailingdataavmedrenewal","id=:id",$bind);
	}
	...
	return HDB::hus()->Hselect("mailingdataavmedrenewal","$where LIKE :svalue LIMIT $limit",$bind);		
	}
	
	// TRUNCATE TABLE
	public function TruncateTable($redirect_to)
	{
   	$sql=HDB::hus()->prepare("TRUNCATE mailingdataavmedrenewal");
	$sql->execute();
	send_to($redirect_to);
	}
	...
	{
	$bind = array(":id" =>$id);
	HDB::hus()->Hdelete("mailingdataavmedrenewal","id=:id",$bind);
	send_to($redirect_to);
	}
	...
	HDB::hus()->Hinsert('mailingdataavmedrenewal', $values);
	}
	...
	HDB::hus()->Hupdate('mailingdataavmedrenewal',$sql,$data);
